...
| 2026-02-17 | CI release workflow fix | Switched `git tag -l --format` → `git for-each-ref --format` in `.github/workflows/release.yaml` for reliable tag annotation extraction. Added validation that fails workflow if release notes file is empty (catches lightweight tags). |
| 2026-02-17 | CI paths-ignore optimization | Added `paths-ignore` to `.github/workflows/test.yml` for both `push` and `pull_request` triggers. Docs-only changes (`**.md`, `LICENSE`, `docs/**`, `.goreleaser.yml`, `examples/**`) skip the entire test battery. `workflow_dispatch` retained for manual runs. |
